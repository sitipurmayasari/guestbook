<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-bold text-slate-800 dark:text-slate-100 text-2xl">WA BLAST — Kirim Pesan WhatsApp ke Tamu</h2>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 mb-6">
        <header class="px-5 py-3 border-b border-slate-100 dark:border-slate-700">
            <h3 class="font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                ...
                Isi Pesan
                <span class="ml-2 text-xs text-blue-400 normal-case font-normal">(gunakan <code>{nama}</code> untuk nama tamu)</span>
            </h3>
        </header>
        <div class="p-5">
    ...
    <div class="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 mb-6">
        <header class="px-5 py-3 border-b border-slate-100 dark:border-slate-700">
            <h3 class="font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-2 text-sm">
                ...
                Preview Pesan
                <span class="ml-1 text-xs text-slate-400 font-normal">(tampilan kiriman ke tamu)</span>
            </h3>
        </header>
        <div class="p-5">
            <div class="inline-block max-w-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-xl rounded-tl-none px-4 py-3 text-sm text-slate-700 dark:text-slate-200 leading-relaxed shadow-sm">
                {!! $messagePreviewHtml !!}
            </div>
            <p class="mt-2 text-xs text-slate-400 dark:text-slate-500">
                <svg class="inline h-3 w-3 mr-1 text-blue-400" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
                </svg>
                Link URL akan otomatis dapat diklik oleh penerima di WhatsApp.
            </p>
        </div>
    </div>
    @endif
        ...
        <div class="p-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select wire:model="period" class="form-select rounded-md shadow-sm block w-full dark:bg-slate-700 dark:border-slate-600 dark:text-white">
    ...
    <div class="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 mb-6">
        <header class="px-5 py-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
            <h3 class="font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                ...
                Data Nomor Telepon Hasil Filter
            </h3>
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    Total: <strong class="text-slate-700 dark:text-slate-200">{{ $visitors->total() }}</strong> tamu &nbsp;|&nbsp;
            ...
            <table class="table-auto w-full dark:text-slate-300 text-sm">
                ...
                <tbody class="text-sm font-medium divide-y divide-slate-100 dark:divide-slate-700">
                    @forelse ($visitors as $index => $visitor)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition {{ in_array((string)$visitor->id, $selectedStr) ? 'bg-green-50 dark:bg-green-900/20' : '' }}">
                            <td class="p-2 text-center">
                                <input type="checkbox"
                                    wire:click="toggleSelected({{ $visitor->id }})"
                                    {{ in_array((string)$visitor->id, $selectedStr) ? 'checked' : '' }}
                                    class="rounded border-slate-400 text-green-500 focus:ring-green-400">
                            </td>
