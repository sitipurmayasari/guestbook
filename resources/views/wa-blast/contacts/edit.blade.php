...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-3xl mx-auto">
    ...
    <div class="mb-6 flex items-center gap-3">
        ...
        <h2 class="font-bold text-slate-800 dark:text-slate-100 text-2xl">Edit Kontak</h2>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
        ...
        <form action="{{ route('wa-blast.contacts.update', $waBlastContact) }}" method="POST" class="p-5 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $waBlastContact->name) }}"
            ...
            <div>
                ...
                <input type="text" name="origin" value="{{ old('origin', $waBlastContact->origin) }}"
            ...
            <div>
                ...
                <input type="text" name="telp" value="{{ old('telp', $waBlastContact->telp) }}"
            ...
            <div>
                ...
                <select name="category"
                    ...
                    <option value="1" {{ old('category', $waBlastContact->category) == '1' ? 'selected' : '' }}>Pengujian</option>
                    <option value="2" {{ old('category', $waBlastContact->category) == '2' ? 'selected' : '' }}>Informasi</option>
                    <option value="3" {{ old('category', $waBlastContact->category) == '3' ? 'selected' : '' }}>Umum</option>
                </select>
                @error('category')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div class="flex items-center gap-3">
                ...
                <input type="checkbox" name="is_active" id="is_active" value="1"
                       ...
                       {{ old('is_active', $waBlastContact->is_active ? '1' : '0') == '1' ? 'checked' : '' }}>
            ...
            <div>
                ...
                <textarea name="note" rows="3"
                          ...
                          placeholder="Catatan tambahan (opsional)">{{ old('note', $waBlastContact->note) }}</textarea>
                @error('note')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
